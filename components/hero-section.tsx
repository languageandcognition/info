...
export function HeroSection() {
  ...
  useEffect(() => {
    ...
    const addNewChar = () => {
      ...
      setFloatingChars((prev) => [...prev, newChar].slice(-50)) // Keep max 50 chars
    }

    // Add new characters every 300ms (faster than original 500ms)
    const interval = setInterval(addNewChar, 300)
      ...
      <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          {floatingChars.map((char) => (
              <div
                  key={char.id}
                  className="absolute text-4xl text-white/10 animate-float-random"
                  style={{
                    left: `${char.x}%`,
                    top: `${char.y}%`,
                    animationDelay: `${char.delay}s`,
                    fontFamily: "serif",
                    animationDuration: `${3 + Math.random() * 2}s`, // Faster duration between 3-5s (was 5-8s)
                  }}
              >
                {char.char}
              </div>
        ...
        {/* CSS for random floating animation */}
        <style jsx>{`
          @keyframes floatRandom {
            ...
            80% {
              opacity: 0.1;
              transform: translate(${Math.random() * 100 - 50}px, ${Math.random() * 100 - 50}px); // Larger random movement (was 60/-30)
            }
            100% {
              transform: translate(${Math.random() * 120 - 60}px, ${Math.random() * 120 - 60}px); // Larger final movement (was 80/-40)
              opacity: 0;
            }
          }
